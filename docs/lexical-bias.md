...
- **Selection Bias**: The model only sees query-document pairs that *already* pass the BM25 filter. It never learns to bridge the "Semantic Gap" because it is never exposed to it during training.
- **Reward Overfitting**: The cross-entropy loss encourages the model to find the easiest signal to distinguish positives from negatives. In a biased dataset, the easiest signal is often sub-word overlap.
...
During training, embeddings often collapse into a narrow cone in the vector space.
- **Mechanism**: If all vectors have high average cosine similarity (e.g., > 0.9), the "Angular Resolution" of the model is lost. This is often caused by the vocabulary bias of the underlying LLM (e.g., BERT/RoBERTa).
- **Impact**: The model might place "iPhone" and "Laptop" close together simply because they both share a "Tech" context, failing to maintain the distance required for precise retrieval.
